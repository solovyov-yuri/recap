...
Саммари форматируется под Telegram Markdown (`*жирный*`, списки с `-`):

```
*Тема встречи*
Обсуждение оптимизации платформы и снижения затрат.

*Ключевые обсуждения*
- Переход на более дешёвую инфраструктуру
- Проблемы с устаревшими потоками данных

*Решения и задачи*
- Проверить статус потоков у Виталия Кондреева
- Исправить тип поля TimeZ → Date в таблице RDV
```
...
Тесты покрывают форматтеры, парсинг транскрипций и pipeline (без обращения к GPU или Ollama).
